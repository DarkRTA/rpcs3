...
void midi_creator::refresh_list()
{
	...
	if (!midi_in->ok)
	{
		cfg_log.error("Could not get MIDI port count.");
		return;
	}
	...
	for (s32 port_number = 0; port_number < port_count; port_number++)
	{
		...
		{
			cfg_log.error("Error getting midi port name for port %d", port_number);
			continue;
		}

		cfg_log.notice("Found midi device with name: %d", buf);
